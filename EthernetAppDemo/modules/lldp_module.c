...
static void lldp_init(App* app) {
    /* Hardware LLDP filtering (PMEN) */
    enc28j60_enable_lldp_filter(app->ethernet);
...
static void lldp_cleanup(App* app) {
    /* Restore normal reception mode for other app features */
    enc28j60_restore_default_filter(app->ethernet);
}
