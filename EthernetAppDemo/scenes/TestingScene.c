...
int32_t testing_thread(void* context) {
    ...
    packet_len = sizeof(example_packet3);

    pcap_capture_add_packet(app->file, example_packet3, (uint32_t)packet_len);

    furi_delay_ms(1000);

    packet_len = sizeof(example_packet4);

    pcap_capture_add_packet(app->file, example_packet3, (uint32_t)packet_len);

    furi_delay_ms(500);

    packet_len = sizeof(example_packet5);

    pcap_capture_add_packet(app->file, example_packet5, (uint32_t)packet_len);

    furi_delay_ms(100);

    packet_len = sizeof(example_packet6);

    pcap_capture_add_packet(app->file, example_packet6, (uint32_t)packet_len);
